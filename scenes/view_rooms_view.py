...
class ViewRoomsView(arcade.View):
    ...
    def _join_server(self, server_data):
        if server_data["has_password"]:
            self.view_setter(
                JoinServerPasswordView(self.resource_manager, self.mods_manager, self.view_setter, self,
                                       server_data,
                                       self.config_manager, self.keyboard_manager, self.mouse_manager))
        else:
            client = GameClient(self.config_manager, self.resource_manager, self.mods_manager,
    ...
    def setup_gui(self):
        ...
        menu_background = self.resource_manager.create_widget("menus_background", size_hint=(0.88, 0.78))
